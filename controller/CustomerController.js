...
//button
let btnAdd = $('#btnCustomerAdd');
let btnUpdateCustomer = $('#btnCustomerUpdate');
let btnDeleteCustomer = $('#btnCustomerDelete');
...
btnUpdateCustomer.click( function () {
    ...
    let option=confirm(`Do you want to Update Item: ${id}`);

    if(option) {
        let res = updateCustomer(id, name, address, salary);
        if (res) {
            alert("Custemer Updated..!");
            $('#tblCustomerBody>tr').on('click', function(){
                let id = $(this).children('td:eq(0)').text();
                let name = $(this).children('td:eq(1)').text();
                let address = $(this).children('td:eq(2)').text();
...
btnDeleteCustomer.click(function () {
    ...
    let option=confirm(`Do you want to Delete Item : ${id}`);
...
function searchId(id) {
    for (let i in customerObjectsArray) {
        if (customerObjectsArray[i].getCustomerId() == id) return customerObjectsArray[i];
    }
    return null;
...
function saveCustomer(id, name, address, salary) {
    ...
    //load customer detail to the item table
    loadAllCustomerToTable();
...
function loadAllCustomerToTable() {
    ...
    //get the one by one object from the array
    for (let i in customer){
        let id = customer[i].getCustomerId();
        let name = customer[i].getCustomerName();
        let address = customer[i].getCustomerAddress();
        let salary = customer[i].getCustomerSalary();
